...
def print_runtime_summary(service="firecrawl"):
    service_name = {
        "firecrawl": "Firecrawl",
        "exa": "Exa",
    }.get(service, "Firecrawl")
    output_file = {
        "firecrawl": "firecrawl-keys.txt",
        "exa": "exa-keys.txt",
...
def upload_key(email, api_key, service="firecrawl"):
    try:
        r = std_requests.post(
...
def do_register(count, delay, upload, service="firecrawl"):
    return do_register_parallel(count, delay, upload, 1, service)

def register_one(index, total, upload, service="firecrawl"):
    print(f"{'='*60}")
...
def do_register_parallel(count, delay, upload, concurrency, service="firecrawl"):
    success = 0
    failed = 0
...
def run_register_flow(count, delay, upload, concurrency, service="firecrawl"):
    if count <= 0:
        print("❌ 注册数量必须大于 0")
        return
    if delay < 0:
